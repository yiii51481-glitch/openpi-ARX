#!/usr/bin/env python3
"""Convert ARX5 single-arm recordings to local LeRobot datasets.

The current single-arm implementation uses the recorder's left-arm pose stream
and left-wrist camera as the single-arm data source. Right-arm pose streams and
right-wrist videos are not used.
"""
...
PROFILES: dict[str, DatasetProfile] = {
    "pick_bread": DatasetProfile(
        repo_id="local/pick_bread_arx5_left_eef",
        output_dir=Path("data/pick_bread_arx5_left_lerobot"),
        sources=(SourceSpec("pick_bread", Path("data/pick_bread")),),
    ),
    "three_tasks": DatasetProfile(
        repo_id="local/pick_bread_mango_bottle_arx5_left_eef",
        output_dir=Path("data/pick_bread_mango_bottle_arx5_left_lerobot"),
        sources=(
            SourceSpec("pick_bread", Path("data/pick_bread"), 100),
            SourceSpec("pick_mango", Path("data/pick_mango")),
            SourceSpec("pick_bottle", Path("data/pick_bottle")),
        ),
    ),
    "six_tasks": DatasetProfile(
        repo_id="local/pick_six_tasks_arx5_left_eef",
        output_dir=Path("data/pick_six_tasks_arx5_left_lerobot"),
        sources=(
            SourceSpec("pick_bread", Path("data/pick_bread"), 100),
            SourceSpec("pick_mango", Path("data/pick_mango")),
            SourceSpec("pick_bottle", Path("data/pick_bottle")),
            SourceSpec("pick_cup", Path("data/pick_cup")),
            SourceSpec("pick_carrot", Path("data/pick_carrot")),
            SourceSpec("pick_pen", Path("data/pick_pen")),
        ),
    ),
}
PROFILES["multitask"] = PROFILES["three_tasks"]
...
def read_pose_csv(path: Path, columns: list[str]) -> np.ndarray:
    """Read one recorder pose stream with the fixed single-arm 10-D layout."""
...
def add_episode(dataset: LeRobotDataset, episode_dir: Path) -> int:
    ...
    state = read_pose_csv(episode_dir / "observation.state.eef_pose" / "data.csv", OBSERVATION_POSE_COLUMNS)
...
def source_episodes(source: SourceSpec) -> list[Path]:
    if not source.directory.is_dir():
        raise FileNotFoundError(f"{source.name}: source directory does not exist: {source.directory}")
    episodes = sorted(path for path in source.directory.glob("episode_*") if path.is_dir())
    if not episodes:
        raise FileNotFoundError(f"{source.name}: no episode directories in {source.directory}")
    return episodes if source.limit is None else episodes[: source.limit]


def create_dataset(repo_id: str, output_dir: Path) -> LeRobotDataset:
...
def convert_profile(
    ...
    *,
    ...
    if source_dir is not None:
        if profile_name != "pick_bread":
            raise ValueError("--source-dir is only supported for the pick_bread profile")
        sources = (SourceSpec("pick_bread", source_dir, max_episodes),)
    elif max_episodes is not None:
        if profile_name != "pick_bread":
            raise ValueError("--max-episodes is only supported for the pick_bread profile")
        sources = (dataclasses.replace(sources[0], limit=max_episodes),)

    target_dir = output_dir or profile.output_dir
    if target_dir.exists():
        raise FileExistsError(f"Output directory already exists: {target_dir}; refusing to overwrite it.")

    selected_sources = [(source, source_episodes(source)) for source in sources]
    total_episodes = sum(len(episodes) for _, episodes in selected_sources)
    dataset = create_dataset(profile.repo_id, target_dir)

    total_frames = 0
    converted = 0
    for source, episodes in selected_sources:
        for episode_dir in episodes:
            frames = add_episode(dataset, episode_dir)
            converted += 1
            total_frames += frames
            logging.info(
                "Converted %d/%d [%s]: %s (%d frames)",
                converted,
                total_episodes,
                source.name,
                episode_dir.name,
